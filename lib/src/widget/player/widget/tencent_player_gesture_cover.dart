...
class TencentPlayerGestureCover extends StatefulWidget {
  ...
  @override
  _TencentPlayerGestureCoverState createState() =>
      _TencentPlayerGestureCoverState();
}
...
class _TencentPlayerGestureCoverState extends State<TencentPlayerGestureCover> {
  ...
  _onVerticalDragStart(DragStartDetails details) async {
    ...
    double xOffSet =
        WidgetUtil.globalOffsetToLocal(currentKey, details.globalPosition).dx;
  ...
  _onVerticalDragUpdate(DragUpdateDetails details) async {
    if (leftVerticalDrag == true) {
      double targetBright =
          ((await Screen.brightness) - details.delta.dy * 0.01).clamp(0.0, 1.0);
      ...
      double targetVolume =
          (currentVolume - details.delta.dy * 0.01).clamp(0.0, 1.0);
